...
export async function generateMetadata({
  ...
  // No fallback title for an unknown slug: a generic title on the not-found
  // page is a soft-404 signal. Same rule as blog/[slug] and projects/[id].
  //
  // Deliberately NOT inside a try/catch, unlike the nine fixed routes this
  // replaced. `notFound()` works by throwing, so catching around it would
  // swallow the 404 and serve a page with generic metadata instead.
  if (!page) notFound();
...
export default async function ServicePage({ params }: ServicePageProps) {
  const { serviceSlug } = await params;
  const [data, grid] = await Promise.all([getPage(serviceSlug), getServiceGrid()]);
  const page = data?.pages?.at(0);
