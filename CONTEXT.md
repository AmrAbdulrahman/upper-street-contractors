...
**Step header**:
The pinned top of a Wizard panel: the stepper and the `Step N of M` eyebrow, and nothing else. The step's heading and hint scroll with the body — they are the question, and the answers belong under it; pinned, they spent a third of a phone's panel on copy read once and scrolled past. What has to stay is which step this is, which the dots and the eyebrow both say (the dots' own labels are `sr-only` below `md`, so the eyebrow is carrying that on a phone). Pinned twice over: first to the panel's own top, and then to the underside of the site header once the page has scrolled the panel up, since a panel roughly a screenful tall has exactly one position at which it would otherwise all be visible. Focus lands on the step's heading on a step change — on the eyebrow only when a step carries no heading, and on the done panel's heading at the end — so what is announced is the question rather than the position, and the page does not move to reveal it.
