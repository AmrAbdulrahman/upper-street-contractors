...
`apps/cms` is the sole network-reachable writer for zero-cms (ADR 0003/0004:
...
`apps/cms/next.config.mjs` deliberately does **not** set `output: "standalone"`.
The git-sync module (`apps/cms/src/lib/zero-cms/git-sync.ts`) needs a real `.git`
...
`npm ci && npx nx build cms` and `npx nx start cms` run **in place** inside
...
1. New service, **Root Directory**: repo root (the whole Nx workspace — `nx build`
   needs it, not just `apps/cms`).
2. **Build command**: `npm ci && npx nx build cms`
3. **Start command**: `npx nx start cms` (runs `next start -p 3001` in place —
   see `apps/cms/project.json`)
...
| Var | Value |
| --- | --- |
| `ZERO_CMS_AUTH_SECRET` | Long random string. Required — cms refuses to boot without it (see `src/lib/zero-cms/server.ts`). |
...
4. **Branch protection on `main`**: this bot pushes directly to `main` (see root
   ...
   token's identity, otherwise every push from cms will be rejected.
...
Staging environment only — production doesn't talk to cms at all (static
export, build-time local-fs read):

| Var | Value |
| --- | --- |
| `ZERO_CMS_REMOTE_URL` | cms's Railway URL, e.g. `https://cms-staging.up.railway.app` |
| `NEXT_PUBLIC_ZERO_CMS_URL` | same URL — this one reaches the browser (Inspect overlay), the other is server-to-server |
| `ZERO_CMS_SERVICE_EMAIL` / `ZERO_CMS_SERVICE_PASSWORD` | A dedicated **viewer-role** account on cms — create it once via `https://<cms-url>/admin` (log in as the seeded admin, create a new user with role `viewer`). Never reuse an Editor's own login for this — it's a server-to-server credential, not a session. |

## Known open item: production static export

Production (`main`, meant to build as `output: "export"`) is **not yet flipped**.
The Strapi-era dynamic routes that used to block this (`/api/auth/*`, `/api/graphql`,
`/api/revalidate`, `/api/strapi-health`, `/api/dev/cms-calls`) are gone now — removed
along with the rest of the Strapi cleanup, not because of static export. What's left,
confirmed by an actual build attempt:
...
Two remaining blockers, both legitimate (not cruft):
- `/api/enquiry` — the contact-form email route, genuinely needs a live server
  (`dynamic = "force-dynamic"`), can't run in a static export as-is.
- `/robots.txt` — reads site config live; needs `export const revalidate = <n>` or
  similar to become export-compatible.

Small, mechanical fixes either way — still a separate scoping exercise from the
zero-cms migration itself. Production currently still runs as a normal Next server
(unaffected either way; it simply never talks to cms).
