...
- **Because of that, `saveSchema`'s destructive-edit guard has to validate the *projected* values, not the raw stored bag.** It originally checked the raw bag, which reported every leftover key as `'Unknown field'` — making it impossible to remove a field from any Type with published entries. Impossible *permanently*, too, not just until a cleanup: the engine hands `publish` the projected bags, so every publish re-materialises every declared field (as `null` when unset) straight back into storage, and no order of operations could clear the key first. Validating what a reader would actually see restores the rule this ADR states while still catching the edits that matter — a stored string under a field that just became `number` is projected, and still fails.
