/**
 * Bounded converters between `blocks` content (the {@link BlocksContent} tree) and
 * HTML, scoped to exactly the node/mark set the {@link ZeroCmsBlocks} renderer
 * supports plus a small round-trip superset. An HTML-based rich-text editor (e.g.
 * HugeRTE) edits HTML; the store keeps blocks. `htmlToBlocks` reports any
 * unsupported nodes it drops so the editor can warn before committing.
 *
 * Pure + framework-agnostic (no React, no editor dep) — safe to export from the
 * barrel. `htmlToBlocks` uses DOMParser → call it client-side only.
 */
...
function blockToHtml(block: BlocksNode): string {
  switch (block.type) {
    case 'paragraph':
...
export function blocksToHtml(blocks: unknown): string {
  ...
  return blocks.map((block) => blockToHtml(block as BlocksNode)).join('');
}
...
export function htmlToBlocks(html: string): { blocks: BlocksContent; dropped: string[] } {
