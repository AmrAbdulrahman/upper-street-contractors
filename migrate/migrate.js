...
function loadConfig(args) {
  ...
  if (!p) return { types: {}, promote: {}, jsonRelations: new Set() };
  const c = JSON.parse(readFileSync(p, 'utf8'));
  return {
    types: c.types || {},
    promote: c.promote || {},
    locale: c.locale,
    jsonRelations: new Set(c.jsonRelations || []),
  };
}
...
async function main() {
  ...
  for (const entry of data.entries) {
    ...
    for (const [fieldId, def] of Object.entries(defs)) {
      if (isEntryLink(def)) {
        const cfIds = def.type === 'Array'
          ? linkIds(field(entry, fieldId, locale))
          : [linkId(field(entry, fieldId, locale))];
        const key = `${ctId}.${fieldId}`;
        if (config.jsonRelations.has(key)) {
          // Polymorphic field — store as ordered JSON array of {type, documentId}
          const items = cfIds
            .map((cf) => {
              const docId = idMap.get(cf);
              if (!docId) return null;
              const refEntry = data.entries.find((e) => e.sys.id === cf);
              const refType = refEntry ? entryContentTypeId(refEntry) : 'unknown';
              return { type: refType, documentId: docId };
            })
            .filter(Boolean);
          if (items.length) relations[fieldId] = items;
        } else {
          // Skip relations where ALL targets are single types — Strapi rejects { set: [...] }
          // against single-type documentIds.
          const resolvedIds = cfIds
            .map((cf) => {
              const docId = idMap.get(cf);
              if (!docId) return null;
              const refEntry = data.entries.find((e) => e.sys.id === cf);
              const refCtId = refEntry ? entryContentTypeId(refEntry) : null;
              if (refCtId && isSingle(refCtId)) return null; // skip single-type targets
              return docId;
            })
            .filter(Boolean);
          if (resolvedIds.length) relations[fieldId] = { set: resolvedIds };
        }
